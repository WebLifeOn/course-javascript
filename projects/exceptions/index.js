...
function isSomeTrue(array, fn) {
  ...
  let flag = false;
  for (let i = 0; !flag && i < array.length; i++) {
    const lol = fn(array[i]);
    if (lol) {
      flag = true;
    }
...
function calculator(number = 0) {
  ...
  const obj = {
    ...
    dif(...arg) {
      let numberDif = number;
      [...arg].forEach((element) => {
        numberDif = numberDif - element;
      });
      return numberDif;
    },
    div(...arg) {
      ...
      [...arg].forEach((element) => {
        if (element === 0) {
          ...
          numberNew = numberNew / element;
        }
      });
      return numberNew;
    },
    mul(...arg) {
      let numberMul = number;
      [...arg].forEach((element) => {
        numberMul = numberMul * element;
      });
      return numberMul;
    },
  };
